...
while (my $line = <IN>){
	...
	# if start coordinate is negative, set it to 0 (R. Oka 2018)
	if( $start < 0 ){
		$start = 0 ;
	}
...
my $out = $out_dir.$sample_name."_".$control_name."_QUAL".$qual."_RD".$min_rd."_VAF".$min_vaf."_GQ".$GQ."_flank".$flank."_MQ".$min_mq."_INDELs_autosomal_noEvidenceControl_temp.vcf";
...
system ("~/.guix-profile/bin/sortBed -i $bed > $bed_sorted");
unlink $bed;

my $bed_merged = $bed_sorted;
$bed_merged =~ s/_sorted.bed/_merged.bed/;
system ("~/.guix-profile/bin/mergeBed -i $bed_sorted > $bed_merged");
unlink $bed_sorted;
...
system ("~/.guix-profile/bin/intersectBed -header -v -a $out -b $bed_merged > $final_out");
...
if ($blacklist ne 0){
    ...
    system ("~/.guix-profile/bin/vcftools --gzvcf $final_out --exclude-positions $blacklist --recode --recode-INFO-all --out $out_blacklist");
...
if ($callable ne 0){
    ...
    system ("~/.guix-profile/bin/intersectBed -header -u -a $final_out -b $callable > $out_call");
